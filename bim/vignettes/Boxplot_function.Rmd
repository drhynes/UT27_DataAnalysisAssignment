...
This function creates a boxplot for each of the numerical (cell) y-variables that are provided (e.g., "Brain_T_Cell_Counts", "Brain_Microglia_Counts", "Brain_Neutrophil_Counts"). These (cell) y-variables are plotted against a single categorical (group) x-variable (e.g., "Age_Class"). There is an option to choose a fill variable to separate and color the boxplots (e.g., by "Genotype") and a facet variable to separate the plots into panels (e.g., by "Sex"). There are also options to add individual data points (e.g. TRUE adds points) and to adjust the whiskers (e.g., 1.5 * IQR is the default). 

```{r}
boxplot_example <- graph_boxplots(
  data = combined_data2, # data 
  x_variable = "Age_Class", # categorical (group) variable
  y_variables = c("Brain_T_Cell_Counts", "Brain_Microglia_Counts", "Brain_Neutrophil_Counts"), # numerical (cell) variables
