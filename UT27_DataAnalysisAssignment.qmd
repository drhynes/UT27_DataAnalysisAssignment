...
old merged code for this section
...
# Get the importance values as a data frame
importance_df <- as.data.frame(sw(model.avg(top_brainCD8_models_list)))

# Add predictor names as a column
importance_df$predictor <- rownames(importance_df)

# Add number of models each predictor appears in
importance_df$N_models <- sapply(rownames(importance_df), function(var) {
  sum(sapply(top_brainCD8_models_list, function(m) var %in% names(coef(m))))
})

# Filter for predictors that appear in all models
n_models_total <- length(top_brainCD8_models_list)
predictors_in_all_models <- importance_df$predictor[importance_df$N_models == n_models_total]

# Create formula using only the consistent predictors
formula_str <- paste("`Brain CD8+ T Cell Counts` ~", paste(predictors_in_all_models, collapse = " + "))
reduced_formula <- as.formula(formula_str)

reduced_model <- lm(reduced_formula, data = combined_data)
...
```{r}
#Determine how well model predicts the reponse
predicted <- predict(reduced_model)
actual <- combined_data$`Brain CD8+ T Cell Counts`
...
plot_model_with_ci <- function(model, data, y_var, x_var) {
  ...
  # Plot
  ggplot(pred_df, aes(x = !!x_sym)) +
    geom_point(aes(y = !!y_sym), alpha = 0.5) +
    geom_line(aes(y = fit), color = "blue") +
    geom_ribbon(aes(ymin = lwr, ymax = upr), fill = "blue", alpha = 0.2) +
    labs(
      title = paste("Predicted", y_var, "with 95% Confidence Interval"),
      x = x_var,
      y = paste("Predicted", y_var)
    )
}
